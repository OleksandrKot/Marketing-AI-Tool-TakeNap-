...
function FilterBarComponent(props: FilterBarProps) {
  const { onFilterChange, className, availableTags = [], selectedTags = [], onTagsChange } = props;
  // `pages` prop is accepted for compatibility with callers but isn't used here yet.
  // mark it as used to avoid linter/type complaints when callers still pass it.
  void props.pages;
  // Quick feature flag: hide tags UI across the app while WIP
  ...
  const [activeDaysOpen, setActiveDaysOpen] = useState(false);

  const handlePlacementFilter = useCallback(
    (placement: string | null) => {
      setSelectedPlacement(placement);
      // Store platform into publisherPlatform so server-side query can filter by publisher_platform
      // Also clear page filter to avoid accidental page_name filtering when selecting placement
      const newFilters = { ...filters, publisherPlatform: placement, page: null };
      try {
        console.debug('Placement filter selected:', placement, newFilters);
      } catch (e) {}
  ...
  const clearFilters = useCallback(() => {
    const newFilters = { search: '', page: null, publisherPlatform: null, date: null, tags: null };
    setFilters(newFilters);
    ...
    >
      <div className="flex flex-col md:flex-row md:justify-between  md:items-center w-full space-y-2 md:space-y-0">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 flex-1">
          {/* Placements Filter */}
          <DropdownMenu open={placementOpen} onOpenChange={(open) => setPlacementOpen(open)}>
            <DropdownMenuTrigger asChild>
              <Button
                onClick={() => {
                  try {
                    // eslint-disable-next-line no-console
                    console.debug('Placement trigger clicked');
                  } catch (e) {}
                  setPlacementOpen((s) => !s);
                }}
            ...
            <DropdownMenuContent className="bg-white border-slate-200 text-slate-800 rounded-xl shadow-lg">
              ...
              {placementOptions.map((placement) => (
                <DropdownMenuItem
                  key={placement.value}
                  onClick={() => {
                    handlePlacementFilter(placement.value);
                    setPlacementOpen(false);
                  }}
                  className={`hover:bg-blue-100 ${
                    selectedPlacement === placement.value ? 'bg-blue-100' : ''
                  }`}
                >
                  <div className="flex items-center justify-between w-full">
          ...
          {/* Active Days Filter */}
          <DropdownMenu open={activeDaysOpen} onOpenChange={(open) => setActiveDaysOpen(open)}>
            <DropdownMenuTrigger asChild>
              <Button
                onClick={() => {
                  try {
                    // eslint-disable-next-line no-console
                    console.debug('ActiveDays trigger clicked');
                  } catch (e) {}
                  setActiveDaysOpen((s) => !s);
                }}
                className="bg-slate-100 hover:bg-slate-200 border-0 text-slate-700 font-medium rounded-xl justify-between h-9 transition-all duration-200 w-full"
              >
                <div className="flex items-center space-x-2">
            ...
            <DropdownMenuContent className="bg-white border-slate-200 text-slate-800 rounded-xl shadow-lg">
              <DropdownMenuItem
                onClick={() => {
                  handleActiveDaysFilter(null);
                  setActiveDaysOpen(false);
                }}
              ...
              {activeDaysOptions.map((option) => (
                <DropdownMenuItem
                  ...
                  className={`hover:bg-blue-100 ${
                    selectedActiveDays === option.value ? 'bg-blue-100' : ''
                  }`}
                >
                  <div className="flex items-center justify-between w-full">
          ...
          {/* Filter by Tags (hidden while WIP) */}
          {SHOW_TAGS && (
            <DropdownMenu open={isTagsDropdownOpen} onOpenChange={setIsTagsDropdownOpen}>
              ...
              <DropdownMenuContent className="bg-white border-slate-200 text-slate-800 rounded-xl shadow-lg w-64">
                <div className="p-2">
                  <div className="text-xs text-slate-500 font-medium mb-2 px-2">
                    Available Tags:
                  </div>
