...
export function PillTypeSelector({
  options,
  selected,
  onChange,
  className,
}: PillTypeSelectorProps) {
  return (
      ...
      <div className="grid gap-2 justify-center bg-slate-100 rounded-xl p-1 grid-cols-[repeat(auto-fit,minmax(120px,1fr))]">
        {options.map((o) => (
          <Button
            ...
            variant="ghost"
            size="sm"
            className={`h-8 px-3 rounded-lg font-medium transition-all duration-200 w-full md:w-auto md:min-w-[120px] whitespace-nowrap ${
              selected === o.value
