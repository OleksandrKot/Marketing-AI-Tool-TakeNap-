...
export function DatePicker({ selectedDateRange, onDateRangeChange, isOpen, onToggle }: DatePickerProps) {
  ...
  const isFirstInRow = (date: Date, calendarDate: Date) => {
    ...
    for (let i = 0; i < dayOfWeek; i++) {
      ...
      if (checkDate.getMonth() === date.getMonth() && isDateInRange(checkDate, dateRange.start, dateRange.end)) {
        return false
      }
    }
    return true
