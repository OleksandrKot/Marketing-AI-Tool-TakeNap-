...
export default function FavoritesModal({ isOpen, onClose, creativeId }: Props) {
  const { collections, createCollection, addToCollection, removeFromCollection, isFavorite, addFavorite, removeFavorite } = useFavorites()
  ...
  const isInCollection = (id: string) => collections.some((c) => c.id === id && c.itemIds.includes(creativeId))
  const isInFolder = (id: string) => (folders.find((f) => f.id === id)?.folder_items || []).some((it) => it.creative_id === creativeId)
  ...
  const toggleFolder = useCallback(async (folderId: string) => {
    try {
      ...
      showToast?.({ message: isInFolder(folderId) ? 'Removed from folder' : 'Added to folder', type: 'success' })
    } catch (e) {
      console.error(e)
          ...
          <div className="flex items-center justify-between p-4 border-b">
            <div className="flex items-center gap-3">
              <div className="rounded-full bg-red-50 p-2">
                <svg className="w-6 h-6 text-red-500" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M12 21s-7.5-4.35-10.5-7.02C-0.46 9.95 2 5 6 5c2.21 0 3.5 1.64 4 2.5.5-.86 1.79-2.5 4-2.5 4 0 6.46 4.95 4.5 8.98C19.5 16.65 12 21 12 21z"/></svg>
              </div>
              <div>
                <h3 className="text-lg font-semibold">Save to...</h3>
                <p className="text-sm text-slate-500">Choose a folder or collection to save this ad. You can create new ones here.</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              }} className={`flex items-center gap-2 ${favoriteState ? 'text-red-600' : 'text-slate-600'}`}>
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill={favoriteState ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 21s-7.5-4.35-10.5-7.02C-0.46 9.95 2 5 6 5c2.21 0 3.5 1.64 4 2.5.5-.86 1.79-2.5 4-2.5 4 0 6.46 4.95 4.5 8.98C19.5 16.65 12 21 12 21z"/></svg>
          ...
          <div className="p-4 grid grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2 max-h-48 overflow-auto mb-3">
                ...
                {folders.map((f) => (
                  <div key={f.id} className="flex items-center justify-between p-2 rounded-md border bg-white hover:shadow-sm">
                    <div className="flex items-center gap-3">
                      <input type="checkbox" checked={isInFolder(f.id)} onChange={() => toggleFolder(f.id)} />
                      <div>
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleCreateFolder} className="px-4"><Plus className="h-4 w-4 mr-2" />Create</Button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 max-h-48 overflow-auto mb-3">
                ...
                {collections.map((c) => (
                  <div key={c.id} className="flex items-center justify-between p-2 rounded-md border bg-white hover:shadow-sm">
                    <div className="flex items-center gap-3">
                      <input type="checkbox" checked={isInCollection(c.id)} onChange={() => toggleCollection(c.id)} />
                      <div>
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleCreateCollection} className="px-4"><Plus className="h-4 w-4 mr-2" />Create</Button>
              </div>
            </div>
          </div>
        </CardContent>
