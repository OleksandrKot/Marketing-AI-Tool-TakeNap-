...
export default function FolderItemCard({ it, ad, fav, onOpenNote, onRemove }: Props) {
    ...
    <Card key={id} className="bg-white border border-slate-100 rounded-2xl overflow-hidden">
      <CardContent className="flex items-center gap-4 p-4 pt-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Button variant="ghost" onClick={() => onOpenNote(id)}>Note</Button>
          <Button variant="ghost" onClick={() => onRemove(id)} className="text-red-600">Remove</Button>
        </div>
      </CardContent>
    </Card>
  )
