...
export default function PersonasSettingsPage() {
  ...
  const handleCopyToProfile = async (persona: Persona) => {
    try {
      ...
      if (!user) {
        alert('Please sign in to copy persona to your profile');
        return;
      }
      ...
      alert('Failed to copy persona to profile: ' + formatError(e));
    }
  };
  ...
  const handleCreatePersona = () => {
    ...
    (async () => {
      try {
        ...
        log.debug('[Personas] supabase insert failed, falling back to localStorage', e);
        try {
          setSupabaseError(formatError(e));
        } catch {}
      }
      ...
      try {
        const existing = JSON.parse(localStorage.getItem('local_personas') || '[]') as Persona[];
        const updated = [...existing, persona];
        localStorage.setItem('local_personas', JSON.stringify(updated));
        setPersonas(updated);
      } catch (e) {
        setPersonas((p) => [...p, persona]);
      }
  ...
  const handleSaveEdit = () => {
    ...
    (async () => {
      try {
        ...
        log.debug('[Personas] supabase update failed, falling back to localStorage', e);
        try {
          setSupabaseError(String(e));
        } catch {}
      }
      ...
      try {
        const existing = JSON.parse(localStorage.getItem('local_personas') || '[]') as Persona[];
        const updated = existing.map((p) => (p.id === editingPersona.id ? editingPersona : p));
        localStorage.setItem('local_personas', JSON.stringify(updated));
      } catch (e) {
        // ignore
      }
    ...
    if (confirm('Are you sure you want to delete this persona?')) {
      (async () => {
        try {
          ...
          log.debug('[Personas] supabase delete failed, falling back to localStorage', e);
          try {
            setSupabaseError(String(e));
          } catch {}
        }
          ...
          <Card className="border-red-200 bg-red-50 rounded-2xl mb-6">
            <CardContent className="p-4 flex items-start justify-between">
              <div>
                ...
                <pre className="text-xs text-red-700 mt-2 whitespace-pre-wrap">{supabaseError}</pre>
              </div>
              <div className="pl-4">
                <Button
                  variant="ghost"
