...
export async function POST(request: Request) {
  try {
    ...
    const { data: folder, error: folderError } = await supabase.from("folders").select("owner").eq("id", folderId).single()
    if (folderError) {
      console.error("folder lookup failed", folderError)
      return NextResponse.json({ error: folderError.message || String(folderError) }, { status: 500 })
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: updateError.message || String(updateError) }, { status: 500 })
    }
